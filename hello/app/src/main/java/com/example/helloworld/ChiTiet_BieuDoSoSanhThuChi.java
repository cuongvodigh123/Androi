        ...
        for(int i = 0 ; i < listgd.size(); i++) {
            ...
            if(cmp(c, timeStart) >= 0 && cmp(c, timeEnd) <= 0) {
                ...
                if (listTime.contains(time)) {
                    ...
                    float income1 = barChartItems.get(index).getIncome() + income;
                    float expense1 = barChartItems.get(index).getExpense() + expense;

                    float profit1 = 0, loss1 = 0;
                    if(income1 > expense1) {
                    ...
                    barChartItems.set(index, new MucBieuDoSSTC(time, income1, expense1, profit1, loss1, false));
                } else {
                    listTime.add(time);
